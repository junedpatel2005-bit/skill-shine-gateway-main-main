...
const OTP_EXPIRATION_MS = OTP_EXPIRATION_MINUTES * 60 * 1e3;
...
async function sendOtpEmail(purpose, email, subject, buildText) {
  ...
  await transporter.sendMail({
    ...
    html: buildOtpEmailHtml({
      ...
      intro: purpose === "SIGNUP" ? "Use this one-time code to finish creating your Servio account." : "Use this one-time code to reset your Servio password."
    })
  });
...
function verifyOtp(purpose, email, otp) {
...
async function sendSignupOtpEmail(email) {
  return sendOtpEmail(
    "SIGNUP",
    email,
